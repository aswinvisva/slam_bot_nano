...
class PointMap:
    ...
    def __init__(self, grid_size=(250, 250), grid_unit_size=0.02,):
        ...
        self.recent_R = np.eye(3)
        self.recent_t = np.zeros((3, 1))

        grid_x_size = int(grid_size[0] / grid_unit_size)
        grid_y_size = int(grid_size[1] / grid_unit_size)

        x = np.linspace(-grid_x_size * grid_unit_size / 2, grid_x_size * grid_unit_size / 2, num=grid_x_size + 1)
        y = np.linspace(-grid_y_size * grid_unit_size / 2, grid_y_size * grid_unit_size / 2, num=grid_y_size + 1)
        self.occupancyGridVisited = np.ones((grid_x_size + 1, grid_y_size + 1))
        self.occupancyGridTotal = 2 * np.ones((grid_x_size + 1, grid_y_size + 1))
        x = np.linspace(-grid_x_size * grid_unit_size / 2, grid_x_size * grid_unit_size / 2, num=grid_x_size + 1)
        y = np.linspace(-grid_y_size * grid_unit_size / 2, grid_y_size * grid_unit_size / 2, num=grid_y_size + 1)
    ...
    def spokesGrid(self, numSpokes):
        ...
        bearingIdxGrid[:, numHalfElem + 1: 2 * numHalfElem + 1] = np.rint((np.pi / 2 + np.arctan(
            yGrid[:, numHalfElem + 1: 2 * numHalfElem + 1] / xGrid[:, numHalfElem + 1: 2 * numHalfElem + 1]))
                / np.pi / 2 * numSpokes - 0.5).astype(int)
        bearingIdxGrid[:, 0: numHalfElem] = np.fliplr(np.flipud(bearingIdxGrid))[:, 0: numHalfElem] + int(numSpokes / 2)
    ...
    def update(self, point_cloud, idx, R, t):
        ...
        numSpokes = int(np.rint(2 * np.pi / angularStep))
        spokesStartIdx = numSpokes - int(((numSpokes / 2 - numSamplesPerRev) / 2) % numSpokes)

        spokesOffsetIdxByTheta = int(np.rint(yaw / (2 * np.pi) * numSpokes))
        x, y = t[0], t[1]
        ...
        for i in range(numSpokes):
            spokeIdx = int(np.rint((spokesStartIdx + spokesOffsetIdxByTheta + i) % numSpokes))
            xAtSpokeDir = radByX[spokeIdx]
            yAtSpokeDir = radByY[spokeIdx]
            rAtSpokeDir = radByR[spokeIdx]
            if point_cloud[i][0] < self.lidarMaxRange:
                emptyIdx = np.argwhere(rAtSpokeDir < point_cloud[i][0] - self.wallThickness / 2)
            else:
                emptyIdx = []
    ...
    def array(self):
        return np.array([p._x for p in self._map if p.n_observations >= 0])
